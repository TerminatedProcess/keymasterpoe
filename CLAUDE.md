...
- **All group editing is VAULT-only.** VAULT is the master library; PUBLIC/PROJECT are deployments (conceptually symlinks of the vault's keys), so create/modify/remove of group membership and disband happen only in the VAULT pane. Deployment panes can *view* groups (drill in) and *receive* a pushed group, nothing more.
...
- **Disband**: `d` on a `▸` header deletes the group definition only (keys are kept, become ungrouped). **Only allowed from the VAULT pane** — the definition is global, so disbanding from PUBLIC/PROJECT would wipe it everywhere; those panes reject `d` on a header. `d` on a key still deletes the key (scope rules unchanged).
